...
pub fn create_hermes_session(session_id: &str, workspace: &str) -> Result<(), String> {
    let tmux_name = tmux_session_name(session_id);
    if tmux::session_exists(&tmux_name) {
        tmux::kill_session(&tmux_name)?;
    }
    tmux::create_tmux_session(&tmux_name, workspace, "hermes")?;
    Ok(())
}
...
fn tmux_session_name(session_id: &str) -> String {
    format!("ac_her_{}", &session_id[..8.min(session_id.len())])
}
...
mod tests {
    ...
    #[test]
    fn test_hermes_session_lifecycle() {
        let tmpdir = std::env::temp_dir().join("ac_test_hermes");
        let _ = std::fs::create_dir_all(&tmpdir);
        let sid = "sess_her_001";

        if session_exists(sid) {
            stop_session(sid).ok();
        }

        let result = create_hermes_session(sid, tmpdir.to_str().unwrap());
        assert!(result.is_ok(), "hermes session created: {:?}", result);
